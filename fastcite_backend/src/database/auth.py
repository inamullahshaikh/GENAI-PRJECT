...
from database.mongo import users_collection, db, pending_admin_signups_collection
...
async def signup(request: SignupRequest):
    ...
    If role is 'admin', it goes to pending admin signups instead.
    """
    ...
    existing_admin_pending = await pending_admin_signups_collection.find_one({"email": request.email})
    ...
    # If admin role is requested, handle differently
    if role == "admin":
        # Store in pending admin signups
        pending_admin_signup_data = {
            "email": request.email,
            "username": request.username,
            "password": request.password,  # Will be hashed on approval
            "name": request.name,
            "dob": request.dob,
            "role": "admin",
        ...
        if existing_admin_pending:
            await pending_admin_signups_collection.update_one(
                {"email": request.email},
                {"$set": pending_admin_signup_data}
            )
        else:
            await pending_admin_signups_collection.insert_one(pending_admin_signup_data)
        ...
        try:
            ...
            for admin in admins:
                try:
                    email_service.send_admin_signup_request_email(
                        admin_email=admin.get("email"),
                        admin_name=admin.get("name", admin.get("username", "Admin")),
                        requester_name=request.name,
                        requester_email=request.email,
                        requester_username=request.username
                    )
                except Exception as e:
                    print(f"⚠️ Failed to send admin notification to {admin.get('email')}: {e}")
        except Exception as e:
            print(f"⚠️ Failed to notify admins: {e}")
        
        return {
            "message": "Verification code sent to your email. Your admin signup request will be reviewed by an administrator after verification.",
    ...
    return {"message": "Verification code sent to your email. Please check your inbox.", "requires_approval": False}

@router.post("/verify-signup-code")
async def verify_signup_code(request: VerifySignupCodeRequest):
    """
    Step 2: Verify the signup code.
    Checks both regular and admin pending signups.
    """
    # Check regular pending signups first
    pending_signup = await pending_signups_collection.find_one({"email": request.email})
    is_admin_signup = False
    
    # Check admin pending signups if not found in regular
    if not pending_signup:
        pending_signup = await pending_admin_signups_collection.find_one({"email": request.email})
...
async def complete_signup(request: CompleteSignupRequest):
    ...
    For regular users: Creates the account immediately.
    For admin users: Marks verification as complete but keeps in pending_admin_signups for approval.
    """
    # Check regular pending signups first
    pending_signup = await pending_signups_collection.find_one({"email": request.email})
    is_admin_signup = False
    ...
    if existing_user:
        ...
        if is_admin_signup:
            await pending_admin_signups_collection.delete_one({"email": request.email})
        else:
            await pending_signups_collection.delete_one({"email": request.email})
        raise HTTPException(status_code=400, detail="User already exists")
    
    # For admin signups, mark as verified and wait for approval
    if is_admin_signup:
        ...
        return {
            "message": "Email verified successfully! Your admin signup request is now pending approval. You will receive an email once an administrator reviews and approves your request.",
            "requires_approval": True
        }
    
    # For regular users, create account immediately
    user_dict = {
        "id": str(uuid4()),
        "username": request.username,
    ...
    return {"message": "Account created successfully! You can now log in.", "requires_approval": False}
