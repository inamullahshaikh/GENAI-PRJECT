...
from app.academic_chunker import AcademicPDFChunker
...
def extract_pdf_metadata_task(pdf_path: str) -> Dict:
    """Extract metadata from PDF. Handles both books with TOC and academic PDFs without TOC."""
    ...
    try:
        toc = doc.get_toc()
        has_toc = bool(toc)  # Check if TOC exists
        ...
        return {
            ...
            "pages": pages,
            "toc": toc if has_toc else [],  # Return empty list if no TOC
            "has_toc": has_toc,  # Flag to indicate TOC presence
...
def extract_pdf_chunks_task(pdf_path: str, toc: List, page_count: int, book_id: str, has_toc: bool = True, workers: int = 6) -> List[Dict]:
    """
    Extract and process PDF chunks using appropriate chunker.
    
    Args:
        pdf_path: Path to PDF file
        toc: Table of contents (empty list if no TOC)
        page_count: Total number of pages
        book_id: Book ID
        has_toc: Whether PDF has a table of contents
        workers: Number of workers (not used but kept for compatibility)
    """
    # Choose chunker based on TOC presence
    if has_toc and toc:
        # Use BookChunker for PDFs with TOC
        chunker = BookChunker(pdf_path, book_id=book_id)
        chunker_type = "BookChunker"
    else:
        # Use AcademicPDFChunker for PDFs without TOC
        chunker = AcademicPDFChunker(pdf_path, book_id=book_id, pages_per_chunk=3, overlap_pages=1)
        chunker_type = "AcademicPDFChunker"
    ...
    try:
        ...
        for chunk in chunks:
            ...
            pdf_chunks.append({
                ...
                "title": chunk["title"],
                "path": chunk["path"],
                "level": chunk.get("level", 1),  # Default to 1 for academic PDFs
                "start_page": chunk["start_page"],
                "end_page": chunk["end_page"],
                "text": chunk["text"],
                "related_paths": chunk.get("related_paths", []),
                "local_path": mini_pdf_path,
                "filename": filename,
            })
        
        print(f"✅ Extracted {len(pdf_chunks)} chunks from PDF using {chunker_type}")
        return pdf_chunks
    finally:
        chunker.close()
...
def process_pdf_task(pdf_path: str, user_id: str, batch_size: int = 50, workers: int = 6):
    ...
    # Step 2.5: Build TOC tree from raw TOC (only if TOC exists)
    has_toc = metadata.get("has_toc", False)
    toc_tree = None
    if has_toc and metadata.get("toc"):
